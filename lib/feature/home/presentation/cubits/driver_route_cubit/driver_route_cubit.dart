...
static const double geofenceRadius = AppConstants.geofenceRadiusMeters;
  ...
  Future<void> fetchDriverRoutes(String driverId) async {
    ...
    result.fold(
      ...
      (routes) => emit(DriverRouteSuccess(
        routes: routes,
...
Future<void> checkDriverLocation() async {
  ...
  for (int i = 0; i < stations.length; i++) {
    ...
    print('📏 Distance to ${stations[i].name}: ${distance.toStringAsFixed(1)}m');
  ...
  emit(currentState.copyWith(
    currentStationIndex: isNear ? nearestIndex! : currentState.currentStationIndex,
